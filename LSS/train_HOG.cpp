...
void compute_hog( const vector< Mat > & img_lst, vector< Mat > & gradient_lst, const Size & size )
{
	HOGDescriptor hog(cvSize(20,20),
		cvSize(10,10), cvSize(10, 10), cvSize(5,5), 9, 1, -1, 0, 0.2, true, 64);
    Mat gray;
...
int main( int argc, char** argv )
{
    ...
    sample_neg( full_neg_lst, neg_lst, Size( 50,50) );
   ...
   // test_it( Size( 20,20 ) ); // change with your 
	Mat img = imread("EXP/test.tiff",1);
	Ptr<SVM> svm;
	// Load the trained SVM.
	svm = StatModel::load<SVM>("my_people_detector.yml");
	// Set the trained svm to my_hog
	vector< float > hog_detector;
	get_svm_detector(svm, hog_detector);
	imshow("gradient", get_hogdescriptor_visu(img.clone(), hog_detector, Size(20,20)));
	waitKey();
...
/*
OpenCV Error : Assertion failed((winSize.width - blockSize.width) % blockStride.
	width == 0 && (winSize.height - blockSize.height) % blockStride.height == 0) in
	cv::HOGDescriptor::getDescriptorSize, file C : \builds\master_PackSlave - win64 - vc12
	- shared\opencv\modules\objdetect\src\hog.cpp, line 92*/